...
@pytest.mark.asyncio
async def test_pay_and_fetch_failed_settlement_is_not_settled(monkeypatch) -> None:
    """A PAYMENT-RESPONSE reporting success=False must NOT count as settled.

    The header parsing cleanly into a SettleResponse only proves settlement was
    attempted. Money-path reporting must fail closed on SettleResponse.success.
    """
    monkeypatch.setattr(
        x402_services.settings,
        "evm_private_key",
    ...
    # Header parsed fine — settlement is reported — but it reports failure.
    assert result["settlement_parse_error"] is None
    assert result["payment_settlement"] is not None
    assert result["payment_settlement"]["success"] is False
    assert result["payment_settled"] is False