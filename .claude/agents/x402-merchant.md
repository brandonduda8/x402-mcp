...
tools: mcp__x402__build_seller_requirements, mcp__x402__verify_payment_payload, mcp__x402__get_pro_upgrade_requirements, mcp__x402__get_tool_credits_requirements, mcp__x402__get_supported_networks, Read, Write
model: sonnet
---
...
3. **Monetization tiers.** Use `get_pro_upgrade_requirements` / `get_tool_credits_requirements` to produce upgrade offers ($29 Pro / $1 per 100 credits by config). Remember these live in the in-memory store — a server restart wipes tier activations and credit balances. Do not sell Pro/credits to real buyers until `REDIS_URL` is set; flag this loudly if asked to.
