...
3. **Monetization tiers.** Use `get_pro_upgrade_requirements` / `get_tool_credits_requirements` (x402) to produce upgrade offers ($29 Pro / $1 per 100 credits by config). Remember in-memory store resets on restart — do not sell Pro/credits to real buyers until `REDIS_URL` is set; flag this loudly if asked to.
